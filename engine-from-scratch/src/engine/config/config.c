...
static char *config_get_value(const char *config_buffer, const char *value){
    ...
    if (!line) {
        ERROR_EXIT("Could not find config value: %s. Try deleting the config.ini and restarting\n", value);
    }
    ...
    ++curr; // Consume "="
    while (*curr == ' ') { // Consume any space
        ++curr;
    }

    // Get characters until end of line or buffer limit
    while (*curr != '\n' && *curr != 0 && curr != end && (tmp_ptr - tmp_buffer < sizeof(tmp_buffer) - 1)){
        *tmp_ptr = *curr++;
        ++tmp_ptr;
    }
...
int config_load(void) {
    File file_config = to_file_read("./config.ini");
...
void config_init(void){
    ...
    to_file_write((void*)CONFIG_DEFAULT, strlen(CONFIG_DEFAULT), "./config.ini");
